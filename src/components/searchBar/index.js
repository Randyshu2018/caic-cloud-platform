...
@Form.create()
class EJUSearchBar extends React.Component {
  constructor(props) {
    super(props);
  ...
  renderSubmit() {
    return (
      <Row gutter={24} className={'EJU-search-bar__submit'}>
        <Col span={24}>
          ...
          {this.props.columns.length > this.state.initialItemCount && (
            <a
              href="javascript:;"
              className={'EJU-search-bar__up-down'}
  ...
  render() {
    ...
    const css = classnames('EJU-search-bar', this.props.className);
        ...
        <Spin spinning={this.state.loading === true}>
          <Form onSubmit={this.onSubmit}>
            <div className="eju-flex">
              <div className="eju-flex-1">
                <Row gutter={24}>{cols}</Row>
              </div>
              <div className="opt">{this.renderSubmit()}</div>
            </div>
          </Form>
        </Spin>
      </div>
